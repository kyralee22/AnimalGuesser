...
public class animalGuesserShell
{
   ...
   public static void main(String argv[])throws IOException
   {
     ...
     // animalGuesserShell x = new animalGuesserShell(); // heap array of questions
      String [] heapQs = readFile("animal.txt"); // fills heap
      System.out.println("Think of an animal...");
      String answer = "";
      int index = 1; // start at first question
   
      while(index < heapQs.length && !heapQs[index].equals("0")) // while... not at the last question
      {
         String question = heapQs[index];
      ...
      if(isNo(answer)) // if last answer was no
      {
         if(index >= heapQs.length) // if at end of heap
         {
            System.out.println("Type something this animal has that your animal doesn't.");
            String newQs = in.nextLine();
               
            int newLen = index * 2 + 1;
               
            System.out.println("What is your animal?");
            String newAnim = in.nextLine();
               
            String [] temp = new String[newLen]; // new heap array with index existing
            for(int i = 0; i < heapQs.length; i++)
               temp[i] = heapQs[i];
            
            temp[index].equals("Does it have " + newQs + "?");
            temp[index * 2].equals("0");
            temp[index * 2 + 1].equals("Is it a " + newAnim + "?"); // places animal in 'yes' spot
               
            heapQs = new String[temp.length];
               
            for(int z = 0; z < temp.length; z++)
               heapQs[z] = temp[z];
         }
      
         if(heapQs[index].equals("0")); // if question empty 
         {
            System.out.println("Type something this animal has that your animal doesn't.");
            String newQs = in.nextLine();
               
            heapQs[index].equals("Does it have " + newQs + "?");
         }
         writeToFile(heapQs, "animal.txt");
      }
